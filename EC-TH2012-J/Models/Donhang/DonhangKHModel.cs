...
namespace EC_TH2012_J.Models
{
    public class DonhangKHModel
    {
        ...
        //get list new order
        public List<DonHangKH> Donhangmoi(int numOfOrder)
        {
            using (Entities db = new Entities())
            {
                List<DonHangKH> listDh = new List<DonHangKH>();
                db.DonHangKHs.AsNoTracking();
                var danhsach = (from p in db.DonHangKHs where p.TinhTrangDH == 1 select p).Take(numOfOrder);
                foreach (var temp in danhsach.ToList())
                {
                    listDh.Add(temp);
                }
