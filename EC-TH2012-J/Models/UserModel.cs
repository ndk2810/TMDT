    ...
    public class UserModel
    {
        ...
        internal void SendMailConfirm(string p, string url)
        {
            ...
            if (us != null)
            {
                ...
                string sub = "[Xác nhận email] Xác nhận đăng ký tại Mobile shop";
                string bo = "";
                bo += "Xin chào " + us.HoTen + ",<br>";
                bo += "Cảm ơn bạn đã đăng ký thành viên tại MobileShop, đây là link xác nhận email của bạn <br>";
        ...
        public AspNetUser Khachhangmoi()
        {
            using (Entities db = new Entities())
            {
                db.AspNetUsers.AsNoTracking();
                var result = (from p in db.AspNetUsers
                              select new
                              {
                                  p.HoTen,
                                  p.Email,
